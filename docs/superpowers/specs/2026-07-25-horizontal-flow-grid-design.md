...
> Naming was a placeholder while this was written; `@flowgrid` and the `flow-` element prefix were subsequently adopted as final.
...
packages/core/src/
  ...
  components/   flow-grid, flow-instance, flow-row, flow-cell, flow-header-cell
...
| -------------------- | --------------------------------------- | ----------------- |
| `<flow-grid>`        | host, controller, scroller, virtualizer | `gridContext`     |
| `<flow-instance>`    | header + rows for one instance          | `instanceContext` |
| `<flow-row>`         | one `DisplayRow`, reads its row signal  | `rowContext`      |
| `<flow-cell>`        | value resolution + renderer host        | `columnContext`   |
| `<flow-header-cell>` | header label + module header slots      | `columnContext`   |
...
remain supported for trivial cases. `<flow-cell>` instantiates tag-name renderers through a cached
...
- A price tick re-renders only the bound cells — assert via a render counter on `<flow-cell>` that neither
