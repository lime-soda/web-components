...
export async function loadCombinedManifest(): Promise<CombinedManifest> {
  ...
  const manifestPath = resolve(import.meta.dirname, '../../combined-manifest.json');
  ...
  try {
    ...
    console.error('Make sure to run the build process to generate the combined manifest');
