...
function debugLog(message: string, data?: unknown): void {
  if (process.env.DEBUG_TOKENS === 'true') {
    console.log(`[DEBUG TOKENS] ${message}`, data ? JSON.stringify(data, null, 2) : '');
  }
}
...
export function getTokens(category?: string): Record<string, TokenCategory> | TokenCategory | null {
  try {
    const allTokens = getAllTokens();
    ...
    debugLog(`Token lookup result for '${category}':`, result ? 'found' : 'not found');
    return result;
  } catch (error) {
...
export async function getCssVariables(): Promise<CSSVariable[]> {
  if (cssVariablesCache) {
    debugLog('Returning cached CSS variables', `${cssVariablesCache.length} variables`);
  ...
  try {
    ...
    try {
      ...
      const tokensPackagePath = require.resolve('@lime-soda/tokens/package.json');
    ...
    if (cssFilePath) {
      try {
        ...
        console.warn('Failed to read CSS file, falling back to token generation:', fileError);
      }
    }
    ...
    function extractCssVariables(obj: Record<string, unknown>, prefix = ''): void {
      for (const [key, value] of Object.entries(obj)) {
        if (typeof value === 'object' && value !== null) {
          if ('$value' in value && typeof value.$value === 'string') {
            const tokenValue = value as TokenValue;
            const cssVarName = tokenValue.name || `${prefix}${prefix ? '-' : ''}${key}`;
            variables.push({
              name: `--${cssVarName}`,
...
export function searchTokens(
  ...
  try {
    ...
    function searchInCategory(obj: Record<string, unknown>, category: string, path = ''): void {
      for (const [key, value] of Object.entries(obj)) {
        ...
        if (typeof value === 'object' && value !== null) {
          if ('$value' in value) {
            ...
            const matchesValue = String(tokenValue.$value).toLowerCase().includes(lowerQuery);
            const matchesDescription = tokenValue.$description?.toLowerCase().includes(lowerQuery);
            const matchesName = tokenValue.name?.toLowerCase().includes(lowerQuery);

            if (matchesKey || matchesValue || matchesDescription || matchesName) {
            ...
            searchInCategory(value as Record<string, unknown>, category, currentPath);
          }
        }
      }
    }
