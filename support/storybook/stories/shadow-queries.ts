 ...
 *
 * The slot half was claimed here and not done. A `<slot>` has no children of
 * its own — what it shows lives in the light DOM and is only reachable through
 * `assignedNodes` — so walking childNodes found nothing, and a component named
 * by its slotted content came back nameless. A button whose label is
 * `<ls-button>Buy</ls-button>` could not be found by that name, which is the
 * only name it has.
 */
function deepText(node: Node): string {
  if (node.nodeType === Node.TEXT_NODE) return node.textContent ?? '';

  // What the slot displays, or its fallback content when nothing was assigned —
  // which is what the browser renders, and so what a reader is told.
  if (node instanceof HTMLSlotElement) {
    const assigned = node.assignedNodes();
    const shown = assigned.length > 0 ? assigned : [...node.childNodes];
    return shown.map((child) => deepText(child)).join('');
  }
