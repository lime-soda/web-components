...
describe('theme tokens', () => {
  describe('customPropertyFor', () => {
    it('converts camelCase to a kebab-case custom property', () => {
      expect(customPropertyFor('rowHeight')).toBe('--grid-row-height');
      expect(customPropertyFor('selectionBackground')).toBe('--grid-selection-background');
    });

    it('leaves a single-word token alone', () => {
      expect(customPropertyFor('surface')).toBe('--grid-surface');
    });
