 ...
 * token maps to one `--grid-*` custom property, and every property the components
 * read appears here — the two are kept in step by a test that scans the source
 * for `var(--grid-…)` and fails on anything undeclared.
 *
...
export function customPropertyFor(token: ThemeToken): string {
  return `--grid-${token.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}`;
}
...
/** The `--grid-*` declarations a theme resolves to. Unset tokens are omitted. */
